...
bool solvePnP(const vector<cv::Point3f> & objpoints,const vector<cv::Point2f> &imPoints,const cv::Mat &camera_matrix,const cv::Mat &dist,cv::Mat &r,cv::Mat &t){
    cv::Mat cam32,dist32;
    camera_matrix.convertTo(cam32,CV_32F);
    dist.convertTo(dist32,CV_32F);
    cv::Mat undP;
    cv::undistortPoints(imPoints,undP,camera_matrix,dist);

    Mat model = Mat::zeros( objpoints.size(),3, CV_64F); // 3D points, z is zero
    Mat iprts = Mat::ones( imPoints.size(),3, CV_64F); // 2D points, homogenous points
    ...
    for(int i=0;i<imPoints.size();i++){
        ...
        iprts.at<double>(i,0)=imPoints[i].x;
        iprts.at<double>(i,1)=imPoints[i].y;
