...
std::vector<cv::Mat> IPPE::solvePnP(const std::vector<cv::Point3f>  &objPoints,const  std::vector<cv::Point2f>&imgPoints, cv::InputArray cameraMatrix, cv::InputArray distCoeffs)
{
    ...
    cv::Mat Rvec2,Tvec2;
     ...
     IPPE::solvePoseOfCentredSquare(markerLength, imgPoints,  cameraMatrix, distCoeffs,   Rvec, Tvec,reprojErr1,Rvec2,Tvec2,reprojErr2);
      return {aruco_mm::getRTMatrix(Rvec,Tvec,CV_32F),aruco_mm::getRTMatrix(Rvec2,Tvec2,CV_32F) } ;
}
...
std::vector<std::pair<cv::Mat,double> > IPPE::solvePnP_(const std::vector<cv::Point3f> &objPoints,const  std::vector<cv::Point2f> &imgPoints, cv::InputArray cameraMatrix, cv::InputArray distCoeffs){
    ...
    cv::Mat Rvec2,Tvec2;
     ...
     IPPE::solvePoseOfCentredSquare(markerLength, imgPoints,  cameraMatrix, distCoeffs,   Rvec, Tvec,reprojErr1,Rvec2,Tvec2,reprojErr2);
      return {std::make_pair(aruco_mm::getRTMatrix(Rvec,Tvec,CV_32F),reprojErr1), std::make_pair(aruco_mm::getRTMatrix(Rvec2,Tvec2,CV_32F),reprojErr2) } ;
